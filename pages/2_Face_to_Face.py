...
import pandas as pd
from utils import (
    initialize_session_state, draw_player_card_2, crear_grafico_radar, make_html_comparison_report, 
...
with col_obj:
    nombre_objetivo = st.selectbox('Jugador A:', nombres_disponibles, key='obj_comp')
...
with col_comp:
    ...
    nombre_comparacion = st.selectbox('Jugador B:', nombres_comparacion, key='comp_comp')
...
# --- TABLA COMPARATIVA DE MÉTRICAS ABSOLUTAS ---
st.markdown("---")
...
if st.button("Generar reporte comparativo (HTML)"):
    html = make_html_comparison_report(
         jugador_obj_data, 
        jugador_comp_data, 
        fig, 
        df_comparativa_abs,
    ...
    st.download_button(
        "Descargar Confrontación (HTML)", 
        data=html_bytes, 
        file_name=f"comparativo_{nombre_objetivo}_vs_{nombre_comparacion}.html", 
        mime="text/html",
        help="Descarga el reporte. Ábrelo en el navegador y usa Ctrl+P para guardar como PDF."
    )